...
function test_not_exists {
    ...
    execute "SELECT id FROM conditional_insert_products where name='banana'" > $DIR/run.output
...
function test_exists {
    DIR=$(current_dir ${BASH_SOURCE[0]})
    executeFile $DIR/maybe-insert.sql
    execute "SELECT id FROM conditional_insert_products where name='cherry'" > $DIR/run.output
    count=`grep "row" ${DIR}/run.output | trim`
